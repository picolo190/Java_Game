...
/**
 * This class represents the player. It has a sprite, which renders the player image pixel by pixel and
 * 2 rectangles, one rectangle represents the player and another one for checking collisions
 */
public class Player implements GameObject
{
    ...
    public void update(Game game)
    {
        ...
        if(keyListener.space() && Projectile.delay>20){
            BufferedImage missleImage = game.loadImage("image/Projectile.png");
            Projectile projectile = new Projectile( new Sprite(missleImage), 10,10, game);
            Projectile.delay=0;

            game.getObjects().add(projectile);
        }
        if(keyListener.left())
        {
            newDirection = 1;
        ...
        if(didMove) {
            ...
            Rectangle axisCheck = new Rectangle(collisionCheckRectangle.x, collisionCheckRectangle.y, collisionCheckRectangle.w, collisionCheckRectangle.h);
    ...
    public Rectangle getRectangle() { return playerRectangle;
    }
